...
def get_custom_sub(base_title, full_sub):
    ...
    while not input_success:
        custom_sub_len = input(
            "Enter desired subtitle length: [0] (drop sub), [1-n] (1-n words), [a] all.\n"
        )
...
def write_book_md(title, author, book_md):
    ...
    if invalid_name:
        print(
            f"Rewriting invalid {book_file_name} | {len(book_file_name)} | it has invalid characters"
        )
...
def main():
    ...
    parser.add_argument("csv", help="Goodreads CSV export file to import")
    parser.add_argument(
        "--template",
        help="Book Markdown template file with $variables. Uses book.md.Template by default.",
    )
    parser.add_argument("--out", help="Output directory. Uses current dir as default.")
    parser.add_argument(
        "--sub_len",
    ...
    with open(csv_path, newline="") as csv_file:
        ...
        for count, row in enumerate(reader):
            ...
            if len(row) != 24:
                print(
                    f"Something went wrong with the column count: Got {len(row)}, expected 24. We should skip {row['Title']}."
                )
                continue
            ...
            if alias and len(base_title) < len(full_title):
                print(
                    f"Alias is true and base_title is shorter, setting alias to {base_title}"
                )
